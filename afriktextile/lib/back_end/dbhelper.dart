...
class DBHelper {
  ...
  Future<List<Commande>> getProducts(String table, int id) async {
    ...
    List<Map> list =
        await dbConnexion.rawQuery('SELECT * FROM $table WHERE userId=?', [id]);
  ...
  void addNewCommand(Products product, String table, int id) async {
    var dbConnexion = await db;
    List<Map> list = await dbConnexion
        .rawQuery('SELECT * FROM $table WHERE prodId=?', [product.id]);
    if (list.length == 0) {
      String query =
          'INSERT INTO $table(userId ,name, qty, price, prodId, image, description, descriptionshort) VALUES(?,?,?,?,?,?,?,?)';
  ...
  void addJsonData(Products product, String table) async {
    ...
    if (list.length == 0) {
      String query =
          'INSERT INTO $table(name, qty, price, active, prodId, image,description,descriptionshort, reduction) VALUES(?,?,?,?,?,?,?,?,?)';
      await dbConnexion.transaction((transaction) async {
        return transaction.rawInsert(query, [
          product.name,
          product.quantity,
      ...
      if (product.quantity != "1") {
        ...
        if (eq(arrayForLastData, arrayForNewData) == true) {
          ...
          String query =
              "UPDATE $table SET name=?, qty=?, price=?, image=?, description=?, descriptionshort=?, reduction=? WHERE prodId=?";
          await dbConnexion.transaction((transaction) async {
  ...
  Future<List<Products>> getLocalJson(String table) async {
    var dbConnexion = await db;
    List<Map> list =
        await dbConnexion.rawQuery('SELECT * FROM $table ORDER BY prodId DESC');
    List<Products> datas = new List();
    for (var i = 0; i < list.length; i++) {
      Products data = new Products();
      data.index = list[i]['index'];
  ...
  void addNewCustomer(String aftxlID, String name, String number,
      String password, int con, String link) async {
    var dbConnexion = await db;
    List<Map> list = await dbConnexion
        .rawQuery('SELECT * FROM Customer WHERE number=?', [number]);
    if (list.length == 0) {
      String query =
          'INSERT INTO Customer(aftxlID, last_name, number, password,con, son, link) VALUES(?,?,?,?,?,?,?)';
      await dbConnexion.transaction((transaction) async {
        return transaction
            .rawInsert(query, [aftxlID, name, number, password, con, 1, link]);
      });
    }
  }
  ...
  Future<List<Customer>> getCustomer() async {
    var dbConnexion = await db;
    List<Map> list =
        await dbConnexion.rawQuery('SELECT * FROM Customer WHERE con=1');
...
//update user info
  ...
  void updateUserInfo(Customer ctm) async {
    var dbConnexion = await db;
    String query =
        "UPDATE Customer SET last_name=?, number=?, password=?, email=?, son=?, link=? WHERE id=?";
    await dbConnexion.transaction((transaction) async {
      return transaction.rawQuery(query, [
        ctm.name,
        ctm.phone,
...
//update LivraisonPlaceInfo
  ...
  Future<int> getCustomerId() async {
    ...
    var dbConnexion = await db;
    var list =
        await dbConnexion.rawQuery('SELECT id FROM Customer WHERE con=1');
  ...
  void updateCommandQty(String ac, int id) async {
    var dbConnexion = await db;
    var list = await dbConnexion
        .rawQuery('SELECT qty FROM Commandes WHERE id=?', [id]);
    for (var i = 0; i < list.length; i++) {
      int nb = list[i]['qty'];
      if (ac == '+') {
